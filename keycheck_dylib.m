//
//  RAMOSS4MAuth.m
//  KeyCheck dylib pronto para compilação
//
...
- (void)handlePan:(UIPanGestureRecognizer *)g {
    ...
    CGPoint trans = [g translationInView:win];
...
- (void)handleTap {
    ...
    UIAlertAction *discord = [UIAlertAction actionWithTitle:@"Discord" style:UIAlertActionStyleDefault handler:nil];
    UIAlertAction *tiktok = [UIAlertAction actionWithTitle:@"TikTok" style:UIAlertActionStyleDefault handler:nil];
...
void showFloatingButton(void) {
    ...
    dispatch_async(dispatch_get_main_queue(), ^{
        ...
        NSDictionary *attrs = @{NSFontAttributeName:[UIFont boldSystemFontOfSize:40], NSForegroundColorAttributeName:[UIColor redColor]};
        NSString *r = @"r";
        CGSize s = [r sizeWithAttributes:attrs];
        CGRect rect = CGRectMake((btn.bounds.size.width - s.width)/2, (btn.bounds.size.height - s.height)/2, s.width, s.height);
        [r drawInRect:rect withAttributes:attrs];
